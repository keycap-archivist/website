...
const Search = () => {
  ...
  return (
    <div className="relative flex flex-col items-center">
      <label
        className={cn(
          'relative block w-[210px] items-center text-sm text-slate-600 shadow-sm transition',
      ...
      >
        ...
        <input
          className={cn(
            'w-[210px] rounded-md border-slate-300/90 pl-9 text-slate-600',
      ...
      {showResult ? (
        <div
          className={cn(
            'absolute z-10 mt-16 max-h-[60vh] w-[90vw] overflow-y-scroll rounded-md bg-white p-6 shadow',
            'scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-300',
            'dark:bg-slate-800 dark:scrollbar-thumb-slate-600 md:mt-16 lg:w-[600px]',
          )}
        >
          <ResultList />
        </div>
